...
class LoginOps {
  private object emailVar extends RequestVar[String]("923933533@qq.com")
    ...
    case Full(user) =>
      <li class="dropdown">
        ...
        <ul class="dropdown-menu">
          ...
          <li><a href="/user/profile">个人资料设置</a></li>
          <li><a href="/notes">记事本</a></li>
          <li><a href="/user/favorites">我的收藏</a></li>
          <li class="last"><a href="/user/sign_out">退出</a></li>
        </ul>
      </li>
    case _ =>
      <li class="sign_up"><a href="/sign_up">注册</a></li>
      <li class="sign_in"><a href="/sign_in">登录</a></li>
  }
  ...
  private def process() = {
    if (emailVar.is.isEmpty) formError("email", "请输入注册的邮箱地址") else {
      User.find(By(User.email, emailVar.is)) match {
        case Full(user) => {
          if (passwordVar.is.isEmpty) formError("password", "请输入你的密码") else {
